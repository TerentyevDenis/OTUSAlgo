...
import static Sorting.ShellSort.shellSort3;

public class OptimisedMergeFileSort {

    final int SORTING_SIZE_CHUNK = 10000000;
    ...
    private void sort(int leftElem, int rightElem) {
        if (rightElem - leftElem + 1 <= SORTING_SIZE_CHUNK) {
            sortingFile(leftElem, rightElem);
            return;
        } else {
    ...
    private void sortingFile(int leftElem, int rightElem) {
        int[] arr = new int[rightElem - leftElem + 1];
        try (FileInputStream fis = new FileInputStream("numberSorts/" + leftElem + "." + rightElem);
             BufferedInputStream bufIL = new BufferedInputStream(fis, 1000);
             DataInputStream dis = new DataInputStream(bufIL)) {
            for (int i = 0; i < arr.length; i++) {
                arr[i] = dis.readUnsignedShort();
            }
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        shellSort3(arr);
        try (FileOutputStream fis = new FileOutputStream("numberSorts/" + leftElem + "." + rightElem)) {
            for (int elem : arr) {
                fis.write(elem / 256);
                fis.write(elem % 256);
            }
    ...
    public static void main(String[] args) {
        ...
        OptimisedMergeFileSort mergeFileSort = new OptimisedMergeFileSort();
